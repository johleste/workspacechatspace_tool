...
# Function to filter non-private spaces
def get_non_private_spaces(spaces):
  """
  Filters a list of spaces to return only non-private ones.

  Args:
    spaces: A list of dictionaries representing chat spaces.

  Returns:
    A list of dictionaries representing non-private chat spaces.
  """
  return [space for space in spaces if space.get('accessState') != 'PRIVATE']

# Function to check space access and build CSV data
def check_spaces_and_build_csv(credentials, csv_file):
  """
  Checks accessState of spaces and builds a CSV with non-private ones.
  ...
  non_private_spaces = get_non_private_spaces(spaces)
  data = []
  for space in non_private_spaces:
...
csv_filename = 'non_private_spaces.csv'
